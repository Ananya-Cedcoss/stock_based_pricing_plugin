...
class WPS_Stripe_Payment_Gateway  {
	...
	public function wps_sfw_generate_payment_request( $order, $source ) {
		...
		$post_data['description'] = sprintf( __( '%1$s - Order %2$s - Upsell Order.', 'subscriptions-for-woocommerce' ), wp_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES ), $order->get_order_number() );
