...
class WPSwings_Onboarding_Helper {
	...
	public function enqueue_styles() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_style( 'wpswings-onboarding-style', WPS_WOCUF_PRO_URL . 'admin/css/wpswings-onboarding-admin.css', array(), '3.5.0', 'all' );

			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_style( 'wpswings-onboarding-select2-style', WPS_WOCUF_PRO_URL . 'admin/css/select2.min.css', array(), '3.5.0', 'all' );
		}
	}
	...
	public function enqueue_scripts() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_script( 'wpswings-onboarding-scripts', WPS_WOCUF_PRO_URL . 'admin/js/wpswings-onboarding-admin.js', array( 'jquery' ), '3.5.0', true );
			...
			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_script( 'wpswings-onboarding-select2-script', WPS_WOCUF_PRO_URL . 'admin/js/select2.min.js', array( 'jquery' ), '3.5.0', false );
		}
	}
	...
	public function add_onboarding_popup_screen() {
		...
		if ( $this->is_valid_page_screen() && $this->can_show_onboarding_popup() ) {
			require_once WPS_WOCUF_PRO_DIRPATH . 'extra-templates/wpswings-onboarding-template-display.php';
		}
	}
	...
	public function add_deactivation_popup_screen() {
		...
		if ( ! empty( $pagenow ) && 'plugins.php' === $pagenow ) {
			require_once WPS_WOCUF_PRO_DIRPATH . 'extra-templates/wpswings-deactivation-template-display.php';
		}
	}
	...
	public function add_on_boarding_form_fields() {
		...
		$fields = array(
			...
			wp_rand() => array(
				'id'          => 'monthly-revenue',
				'label'       => esc_html__( 'What is your monthly revenue?', 'one-click-upsell-funnel-for-woocommerce-pro' ),
				'type'        => 'radio',
			...
			wp_rand() => array(
				'id'          => 'industry_type',
				'label'       => esc_html__( 'What industry defines your business?', 'one-click-upsell-funnel-for-woocommerce-pro' ),
				'type'        => 'select',
				'name'        => 'industry_type_',
				'value'       => '',
				'multiple'    => 'yes',
			...
			wp_rand() => array(
				'id'          => 'onboard-email',
				'label'       => esc_html__( 'What is the best email address to contact you?', 'one-click-upsell-funnel-for-woocommerce-pro' ),
				'type'        => 'email',
				'name'        => 'email',
			...
			wp_rand() => array(
				'id'          => 'onboard-number',
				'label'       => esc_html__( 'What is your contact number?', 'one-click-upsell-funnel-for-woocommerce-pro' ),
				'type'        => 'text',
				'name'        => 'phone',
				'value'       => '',
				'required'    => 'yes',
	...
	private function hic_post( $endpoint, $post_params, $headers ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$status_code = 500;
			$_response   = esc_html__( 'Unexpected Error Occured', 'one-click-upsell-funnel-for-woocommerce-pro' );
			$errors      = $response;
