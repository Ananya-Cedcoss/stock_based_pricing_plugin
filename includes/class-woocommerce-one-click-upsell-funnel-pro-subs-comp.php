 ...
 * @since    3.1.0
 */
function wps_upsell_subs_plugin_active() {
 ...
 * @since    3.1.0
 */
function wps_upsell_order_contains_subscription( $order_id ) {

	if ( empty( $order_id ) || ! wps_upsell_subs_plugin_active() ) {
 ...
 * @since    3.1.0
 */
function wps_upsell_is_subscription_product( $product ) {

	if ( empty( $product ) || ! wps_upsell_subs_plugin_active() ) {
 ...
 * @since    3.1.0
 */
function wps_upsell_subs_supported_gateways() {
 ...
 * @since    3.1.0
 */
function wps_upsell_pg_supports_subs( $order_id ) {
	...
	if ( in_array( $payment_gateway, wps_upsell_subs_supported_gateways(), true ) ) {
 ...
 * @since    3.1.0
 */
function wps_upsell_subscription_error() {
 ...
 * @since    3.1.0
 */
function wps_upsell_create_subscriptions_for_order( $order_id, $order = '' ) {
	...
	if ( empty( $order->get_user_id() ) ) {
		...
		function_exists( 'wps_upsell_create_and_auth_customer' ) && wps_upsell_create_and_auth_customer( $order_id );
	}
	...
	if ( ! empty( $order_items ) && is_array( $order_items ) ) {
		...
		foreach ( $order_items as $single_item_id => $single_item ) {
			...
			$target_item_subs = get_post_meta( $order_id, '_wps_wocufpro_replace_target_subs_id', true );

			if ( empty( $target_item_subs ) && ! empty( $target_item_id ) && (int) $single_item_id === (int) $target_item_id ) {

				update_post_meta( $order_id, '_wps_wocufpro_replace_target_subs_id', $subscription->get_id() );
			}

			if ( is_wp_error( $subscription ) ) {

				wps_upsell_subscription_error();
			}
 ...
 * @since    3.1.0
 */
function wps_upsell_create_subscription_for_upsell_product( $order_id, $product, $quantity = 1 ) {
 ...
 * @since    3.1.0
 */
function wps_upsell_subs_set_price_accordingly( $product ) {
 ...
 * @since    3.5.0
 */
function wps_upsell_offer_is_subscription( $order_items = array() ) {
 ...
 * @since    3.5.0
 */
function wps_upsell_funnel_contains_any_subscription( $order_id = false, $offer_products = false ) {
	...
	if ( false === $result && wps_upsell_offer_is_subscription( $offer_products ) && wps_upsell_pg_supports_subs( $order_id ) ) {
