...
class WPS_Upsell_Widget_Loader {
	...
	const WPS_UPSELL_WIDGET_LOADER = WPS_WOCUF_PRO_DIRPATH . 'page-builders/';
	...
	public function load_builders() {
		...
		if ( ! empty( $builders ) && is_array( $builders ) ) {
			foreach ( $builders as $slug => $class_name ) {
				...
				if ( function_exists( 'wps_upsell_lite_is_plugin_active' ) && wps_upsell_lite_is_plugin_active( $slug ) ) {
					$active = true;
